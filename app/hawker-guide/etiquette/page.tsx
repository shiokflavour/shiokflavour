import Link from "next/link";
import { SiteHeader } from "@/app/components/site-header";
import { SiteFooter } from "@/app/components/site-footer";
import { ETIQUETTE_TIPS } from "@/app/lib/hawker-guide";
import { KeepExploring } from "../keep-exploring";

export const metadata = {
  title: "Hawker Etiquette | The Unwritten Rules of Singapore Hawker Centres",
  description:
    "Choping seats with tissue, ordering at the stall, returning your tray. The unwritten rules nobody tells you — but everyone expects you to know.",
};

export default function EtiquettePage() {
  return (
    <>
      <SiteHeader />

      {/* Hero */}
      <section className="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <Link
          href="/hawker-guide"
          className="inline-flex items-center gap-2 text-sf-muted hover:text-sf-primary text-sm font-semibold mb-8 transition-colors"
        >
          <svg
            className="h-4 w-4"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2.5}
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18"
            />
          </svg>
          Back to Hawker Guide
        </Link>

        <p className="text-[15px] font-semibold tracking-[0.25em] text-sf-primary uppercase mb-6">
          How It Works
        </p>
        <h1 className="text-5xl sm:text-6xl font-bold text-white leading-[1.0] mb-6 max-w-3xl">
          Hawker Etiquette
        </h1>
        <p className="text-sf-muted text-base sm:text-lg leading-relaxed max-w-2xl">
          There are unwritten rules at every hawker centre. Nobody will tell you
          — you are just expected to know. Now you do.
        </p>
        <div className="w-12 h-px bg-sf-primary mt-10" />
      </section>

      {/* Cards */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {ETIQUETTE_TIPS.map((tip, i) => (
            <article
              key={tip.title}
              className="bg-[#1a1a1a] rounded-2xl p-6 border border-white/5 hover:border-sf-primary/30 transition-all"
            >
              <div className="flex items-start gap-4 mb-4">
                <span className="text-sf-primary font-bold text-2xl leading-none mt-1">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <h2 className="text-white font-bold text-lg leading-snug">
                  {tip.title}
                </h2>
              </div>
              <p className="text-sf-muted text-sm leading-relaxed mb-4">
                {tip.description}
              </p>
              {tip.localSay && (
                <div className="bg-black/30 rounded-lg px-4 py-3">
                  <p className="text-white/60 text-[15px] italic">
                    &quot;{tip.localSay}&quot;
                  </p>
                </div>
              )}
            </article>
          ))}
        </div>
      </section>

      <KeepExploring currentId="etiquette" />

      <SiteFooter />
    </>
  );
}
